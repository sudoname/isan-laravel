...
@section('page-title', 'Edit WhatsApp Group')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('admin.whatsapp-groups.update', $whatsappGroup) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="description"
                                  id="description"
                                  rows="3"
                                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('description') border-red-500 @enderror">{{ old('description', $whatsappGroup->description) }}</textarea>
                    ...
                    <div>
                        ...
                        <select name="category"
                            ...
                            @foreach($categories as $cat)
                                <option value="{{ $cat }}" {{ old('category', $whatsappGroup->category) == $cat ? 'selected' : '' }}>{{ $cat }}</option>
                            @endforeach
                        </select>
                        @error('category')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="url"
                               name="invite_link"
                               id="invite_link"
                               value="{{ old('invite_link', $whatsappGroup->invite_link) }}"
                               required
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text"
                                   name="admin_name"
                                   id="admin_name"
                                   value="{{ old('admin_name', $whatsappGroup->admin_name) }}"
                        ...
                        <div>
                            ...
                            <input type="text"
                                   name="admin_phone"
                                   id="admin_phone"
                                   value="{{ old('admin_phone', $whatsappGroup->admin_phone) }}"
                                   placeholder="+234..."
                    ...
                    <div>
                        ...
                        <input type="number"
                               name="display_order"
                               id="display_order"
                               value="{{ old('display_order', $whatsappGroup->display_order) }}"
                               min="0"
                    ...
                    <div class="flex items-center">
                        <input type="checkbox"
                               ...
                               id="is_active"
                               value="1"
                               {{ old('is_active', $whatsappGroup->is_active) ? 'checked' : '' }}
