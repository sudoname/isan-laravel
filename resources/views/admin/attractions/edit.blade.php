        ...
        <form action="{{ route('admin.attractions.update', $attraction) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <select name="category"
                                ...
                                @foreach($categories as $cat)
                                    <option value="{{ $cat }}" {{ old('category', $attraction->category) == $cat ? 'selected' : '' }}>{{ $cat }}</option>
                                @endforeach
                            </select>
                            @error('category')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <textarea name="description"
                                  ...
                                  rows="3"
                                  maxlength="500"
                                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('description') border-red-500 @enderror">{{ old('description', $attraction->description) }}</textarea>
                    ...
                    <div>
                        ...
                        <textarea name="full_description"
                                  id="full_description"
                                  rows="10"
                                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent tinymce @error('full_description') border-red-500 @enderror">{{ old('full_description', $attraction->full_description) }}</textarea>
                    ...
                    <div>
                        <label for="image_url" class="block text-sm font-medium text-gray-700 mb-2">
                            {{ $attraction->image_url ? 'Replace Image' : 'Attraction Image' }}
                        </label>
                        <input type="file"
                               name="image_url"
                               id="image_url"
                    ...
                    <div>
                        ...
                        <input type="number"
                               name="display_order"
                               id="display_order"
                               value="{{ old('display_order', $attraction->display_order ?? 0) }}"
                               min="0"
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center">
                            <input type="checkbox"
                                   ...
                                   id="is_published"
                                   value="1"
                                   {{ old('is_published', $attraction->is_published) ? 'checked' : '' }}
                                   class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
            ...
            <div class="flex items-center justify-between pt-6 border-t">
                ...
                <button type="submit"
                    ...
                    <i class="fas fa-save mr-2"></i> Update Attraction
                </button>
            </div>
        </form>
    </div>
